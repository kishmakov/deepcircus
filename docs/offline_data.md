...
The generator takes a seed and entry count on the command line. Both come from
`conf/preparation.conf`, with one entry count shared by every bitness. Its content
must be a pure function of `(seed, series, bitness, entries)` -- no clock, no global RNG, no filesystem
state -- so a file can be reproduced from the config alone. The seed is *not*
