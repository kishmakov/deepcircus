...
We work over a fixed set of logical elements $\mathcal{K} = \left\{ K_1, K_2, \dots, K_m \right\}$.
...
\emph{slots} -- the signals that are available and not yet consumed -- and $C$ is the
current unfinished circuit. Every slot $y \in S$ is itself a Boolean function of the
primary inputs, so $S$ can be read as a value table with one column per slot. Initially
$$
...
attachment: an element $K \in \mathcal{K}$ of arity $k$, together with a $k$-subset
$\left\{y_{i_1}, \dots, y_{i_k}\right\} \subseteq S$ of slots to feed it. The output of
the element produces a new slot
$$
    y = K\left(y_{i_1}, \dots, y_{i_k}\right),
$$
whose column is computed row-wise from the columns of its inputs, and the candidate
carries the state to
$$
    S_{j+1} = S_j \cup \left\{y\right\},
    \qquad
    C_{j+1} = C_j + K.
$$
...
decision tree that computes $g$ from the value table of $S$. This computation of best
tree is exponential in arity.  While this choice gives good convergence, computing it
quickly becomes impractical as the arity of $g$ grows.
