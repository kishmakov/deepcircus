...
- `cpp/common/` holds the `common` library; `cpp/common/offline/read_write.{h,cpp}` is the shared reader and writer for the packed offline-data format, included as `"offline/read_write.h"`, and `cpp/common/tools/solver.{h,cpp}` owns the exact truth-table solvers
...
- `cpp/preparation/` holds the `offline_train_data_generator` executable that writes random offline training data one bitness at a time (`offline_train_data_generator <output_dir> <bitness> <seed> <entries>` -> `s1_<bitness>_rand.bin`, `s2_<bitness>_rand.bin`, bitness zero-padded to two digits so a listing sorts in bitness order); it uses the specialized exact solvers for the two series, is a subdirectory of the `cpp/` CMake project, and is built by `scripts/build.sh` into `cpp/build/preparation/`; `scripts/preparation/build_offline_generator.sh` builds that one target and publishes it as the `execs/offline_train_data_generator` symlink, which is the name callers use
...
- `scripts/preparation/prepare_offline_train_data.sh` makes `data/s{1,2}_{08..12}_rand.bin` exist: it regenerates only the bitnesses whose pair is incomplete, staging through the work dir and moving the results into `data/`. Its `work_dir`, `seed`, shared entry count, and bitness range all come from `conf/preparation.conf`; a key missing there is fatal rather than defaulted. `data/*` is gitignored apart from `data/circuits/`
