...
- `cpp/common/` holds shared sources; `cpp/common/offline/read_write.{h,cpp}` is the reader and writer for packed offline data, `cpp/common/tools/random.{h,cpp}` owns the shared SplitMix64 primitives and random stream, `cpp/common/tools/solver.{h,cpp}` owns the exact truth-table solvers, and `cpp/common/func/table.{h,cpp}` owns `func::TableCase`, uniformly random materialized truth-table creation, and the `func::Table*` batch APIs and is included as `"func/table.h"`
...
- `cpp/generator/case.{h,cpp}` owns the `Case` base class: per-case deterministic randomness keyed by `(bitness, seed)`, the fair-coin bit stream (`GenerateBool`), and the sampling entry points (`SampleValues`/`SampleRestrictions`/`SampledValueString`); `TableCase`/`TreeCase` supply the virtual `Evaluate(std::vector<bool>)`. A case is its seed and nothing else -- no case id reaches `Case`, and both subclasses draw everything they are (a truth table, a tree's internal-node budget) off that one stream
...
- `cpp/common/tools/solver.{h,cpp}` owns the exact truth-table solvers `tools::SolveForDepth`/`tools::SolveForSize` (the 3^bitness dynamic programs behind a solvable table's targets), their `SolveFor...Given` variants scoring a tree that may also query a second function once per path (the paper's `S_1[g | f]`), their `SolveFor...Restricted` variants scoring a tree that only has to be right on a subset (`S_2[g | X]`), and `tools::kMaxSolvableBitness`, which callers name directly rather than through a per-module alias; included as `"tools/solver.h"` and called from `table.cpp`. `cpp/test/test_solver.cpp` covers it, cross-checking against a brute-force reference at small bitness
- `cpp/generator/generator.h` declares the tree/circuit synchronous API and shared generated tensor types; it includes `tools/sample.h` and re-exports `gen::InputShape` as an alias of `tools::InputShape`; the table synchronous API is declared in `cpp/common/func/table.h`
- `cpp/generator/aig.cpp` locates `data/circuits` relative to its own source path (falls back to walking up from the cwd)
- `cpp/generator/utils.{h,cpp}` owns deterministic per-case seed sampling (`SampleSeeds`/`DomainSeed`/`TaskSeed`) and the case-shaped helpers `FullBitId`/`SizeScore`; its random stream and SplitMix64 primitives come from `cpp/common/tools/random.{h,cpp}`
- `cpp/generator/generator.cpp` owns the compact bit-packed `BitMatrix` (`gen::Values`/`gen::Restrictions`) and the circuit forwarding; `cpp/generator/tree.cpp` and `cpp/common/func/table.cpp` own their `*SampleSeeds`/`*ForSeeds` batch entry points
- `cpp/producer/task_queue.{h,cpp}` owns the wire-level task/tensor types and the task queue; coordinates are produced strictly sequentially, chunking each coordinate's case seeds across the thread pool and merging the chunks with `gen::Values::Concat`/`gen::Restrictions::Concat`
