...
implements: call it on one input or materialize its whole truth table.
`table.h` is the seeded-table kind. `tree.h` is the kind
...
The code is in namespace `func` and compiled into `common`.
