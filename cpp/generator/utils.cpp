...
void FlippingSampler::Fill(std::string &value, size_t sample_offset, size_t fixed_bit_id,
    ...
    for (size_t coord = 0; coord < free_bits; ++coord) {
        ...
        bit = bit == '1' ? '0' : '1';
        value[sample_offset + free_bits + 1 + coord] = evaluate({input.data(), bitness_}) ? '1' : '0';
        bit = bit == '1' ? '0' : '1';
    }
}
...
void FlippingSampler::Fill(float *value, size_t sample_offset, size_t fixed_bit_id,
    ...
    value[sample_offset + free_bits] = evaluate({input.data(), bitness_}) ? 1.0f : -1.0f;
    for (size_t coord = 0; coord < free_bits; ++coord) {
        char &bit = input[FullBitId(coord, fixed_bit_id)];
        bit = bit == '1' ? '0' : '1';
        value[sample_offset + free_bits + 1 + coord] = evaluate({input.data(), bitness_}) ? 1.0f : -1.0f;
        bit = bit == '1' ? '0' : '1';
    }
}
