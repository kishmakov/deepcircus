...
struct AigCircuit {
    ...
    AigCircuit(AigCircuit&& other) noexcept : meta(std::move(other.meta)), aig(other.aig) { other.aig = nullptr; }
...
std::filesystem::path CircuitRoot() {
    const std::filesystem::path source_root =
        std::filesystem::path(__FILE__).parent_path().parent_path().parent_path() / "data" / "circuits";
...
AigHeader ReadAigHeader(std::istream& in) {
    ...
    std::string magic;
    AigHeader parsed;
    header >> magic >> parsed.max_var >> parsed.inputs >> parsed.latches >> parsed.outputs >> parsed.ands;
    if (!header || magic != "aig") {
        throw std::runtime_error("unsupported AIG header");
    }
    header >> parsed.bads >> parsed.constraints >> parsed.justice >> parsed.fairness;
...
bool EvaluateAigLiteral(const AigCircuit& circuit, uint32_t literal, std::vector<int8_t>& cache) {
    ...
    if (var == 0) {
        ...
        if (cached < 0) {
            ...
            assert(gate != nullptr);
            cached = EvaluateAigLiteral(circuit, gate->rhs0, cache) && EvaluateAigLiteral(circuit, gate->rhs1, cache);
        }
        result = cached != 0;
    }
...
void WriteAigOutputs(std::string& value, size_t offset, const AigCircuit& circuit, std::string_view input_state) {
    ...
    for (uint32_t id = 0; id < circuit.aig->num_outputs; ++id) {
        value[offset + output_id] = EvaluateAigLiteral(circuit, circuit.aig->outputs[id].lit, cache) ? '1' : '0';
        ++output_id;
    }
    for (uint32_t id = 0; id < circuit.aig->num_bad; ++id) {
        value[offset + output_id] = EvaluateAigLiteral(circuit, circuit.aig->bad[id].lit, cache) ? '1' : '0';
        ++output_id;
    }
}
...
size_t CircuitInputs(const char* set_name, const char* case_name) { return GetCircuitMeta(set_name, case_name).inputs; }
...
const char* CircuitValue(const char* set_name, const char* case_name, const char* input_state) {
    ...
    for (size_t bit_id = 0; bit_id < input_state_size; ++bit_id) {
        ...
        WriteAigOutputs(value, input_state_size + outputs * (bit_id + 1), circuit, point_input);
        point_input[bit_id] = point_input[bit_id] == '1' ? '0' : '1';
    }
