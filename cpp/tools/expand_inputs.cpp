// Thin stdin/stdout bridge over gen::ExpandInputs: expands per-batch base
...
int main(int argc, char** argv) {
    ...
    while (std::getline(std::cin, line)) {
        sequences.push_back(gen::BitsFromChars(line));
        if (sequences.size() < shape.batches) {
            continue;
        }

        const std::vector<bool> points = gen::ExpandInputs(shape, sequences);
        std::string output;
