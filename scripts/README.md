...
| [`train/train_model.sh`](train/train_model.sh) | `train_model.sh m1 8`, `train_model.sh m2 8` -- trains either model at one bitness on its offline data, per `conf/train.yaml`, leaving the weights and metrics in that config's `work_dir` |
...
| Script | What it does |
| --- | --- |
| [`build.sh`](build.sh) | configures and builds the whole C++ tree in Release |
| [`test.sh`](test.sh) | builds the whole tree with ASan/UBSan and every assert alive, runs the suite, then runs each executable once on a small input; extra arguments go to GoogleTest |
...
| Script | What it does |
| --- | --- |
| [`dimensions.py`](dimensions.py) | rewrites `data/dimensions.txt` from the circuits in `data/circuits/` |
