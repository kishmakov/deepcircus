...
| [`test.sh`](test.sh) | builds the whole tree with ASan/UBSan and every assert alive, runs the suite and executable smoke checks, then runs [`test.py`](test.py) against what they wrote -- the client against the server, and tiny M1 and M2 runs for one epoch checking moment invariance; extra arguments go to GoogleTest |
