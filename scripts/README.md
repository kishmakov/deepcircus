...
| [`train/train_model.sh`](train/train_model.sh) | `train_model.sh m1 8` -- trains one model at one bitness on its offline data, per `conf/train.yaml`, leaving the weights and metrics in that config's `work_dir` |
