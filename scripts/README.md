...
| [`docs/plot_metrics.sh`](docs/plot_metrics.sh) | plots the train and validation curves of every run in that `work_dir`, three PNGs beside each `*.metrics.json` -- both scores, then the depth and size scores alone; `scale=log` puts the error on a logarithmic axis |
