...
| [`train/build_visualizer.sh`](train/build_visualizer.sh) | builds `graph_vis` and publishes it as `execs/graph_vis` |
...
| [`docs/plot_metrics.sh`](docs/plot_metrics.sh) | plots the train and validation curves of every run in that `work_dir`, one PNG beside each `*.metrics.json` stacking three panels -- both scores, then the depth and size scores alone; `scale=log` puts the error on a logarithmic axis |
...
| [`test.sh`](test.sh) | builds the whole tree with ASan/UBSan and every assert alive, runs the suite and executable smoke checks, then runs [`test.py`](test.py) against what they wrote -- the client against the server, and tiny M1 and M2 runs for one epoch checking the predictor's invariances; extra arguments go to GoogleTest |
