...
from src.config import OrderedConfig, TrainConfig
...
def make_predictor(
    ...
    return OrderedRestrictionPredictor(
        config.bitness if bitness is None else bitness,
        config.sampling.points,
        config.model_name if model_name is None else model_name,
        config.model,
    )
